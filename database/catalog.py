...
async def update_category(category_id: ObjectId, name: str):
...
async def create_product(name: str, category_id: str, price: float, description: str = "", stock: int = 0, photo: str = None):
    """
    Створює новий товар.
    """
    product_data = {
...
async def update_product(product_id: str, name: str = None, price: float = None, description: str = None, stock: int = None, photo: str = None):
    update_data = {}
    if name:
    ...
    result = await product_collection.update_one({"_id": ObjectId(product_id)}, {"$set": update_data})
    return result.modified_count
